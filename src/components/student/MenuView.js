...
export default function MenuView() {
    ...
    const fetchMenu = async () => {
      try {
        ...
        const response = await axios.get(
          `/api/menu?day=${currentDay}&weekNumber=${weekNumber}`
        );
  ...
  const handleDayChange = async (day) => {
    try {
      ...
      const response = await axios.get(
        `/api/menu?day=${day}&weekNumber=${weekNumber}`
      );
      setMenu(response.data);
      setLoading(false);
    } catch (err) {
  ...
  return (
    <div className=" text-black bg-white rounded-lg shadow-md p-6">
      ...
      {menu.length === 0 ? (
        <div className=" text-black text-center p-4 bg-gray-100 rounded-md">
