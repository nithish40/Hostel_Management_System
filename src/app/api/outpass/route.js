...
export async function GET(request) {
  try {
    ...
    console.log(user);
    if (user.role === "admin") {
      outpasses = await Outpass.find()
        .sort({ createdAt: -1 })
        .populate("student", "name email roomNumber hostelBlock");
      console.log(outpasses);
    } else {
      outpasses = await Outpass.find({ student: user.id }).sort({
        createdAt: -1,
      });
    }
...
export async function POST(request) {
  try {
    ...
    if (user.role !== "student") {
      return NextResponse.json(
        { error: "Only students can create outpass requests" },
        { status: 403 }
      );
    }
...
export async function PUT(request) {
  try {
    ...
    if (user.role !== "admin") {
      return NextResponse.json(
        { error: "Only admins can update outpass status" },
        { status: 403 }
      );
    }
